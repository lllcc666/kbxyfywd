...
- `400600805/806` 是墨魂战斗后的结果对话；native 需要捕获结果回包，再发送 `TASK_TALK(npcid, [dialogId])` 完成“确定/离开”，之后才进入 `400600802` 收尾。
- `g_eightTrigramsResumeStepIndex` 表示下一个待完成步骤；进入步骤时保持当前索引，只有封包和回包闭环完成后才前移，不能在刚进入场景时提前推进。
- 自动重启按同一断点统计连续失败，采用退避等待；如果检测到残留战斗未退出，则不重复发送任务封包。
...
| `400600805` / `400600806` | 捕获 `TASK_TALK_BACK` -> `TASK_TALK_SEND(npcid, [dialogId])` | 墨魂战斗结果回话的“确定/离开”，完成后再进入 `400600802` |
